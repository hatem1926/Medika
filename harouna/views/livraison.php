...
if(isset($_POST['forminscription'])) 
{
   ...
   $etat =$_POST['etat']; 
   var_dump($_POST);
   if(!empty($_POST['name']) AND !empty($_POST['tel']) AND !empty($_POST['email']) AND !empty($_POST['region'])  AND !empty($_POST['etat'])) {

 
$insertmbr = $bdd->prepare("INSERT INTO livraison(name,tel,email,region,etat) VALUES(?,?,?,?,?)");
$insertmbr->execute(array($name,$tel,$email,$region,$etat));
...
<html>
   <body>
      <div align="center">
         ...
         <p>bonjour mr'.urldecode($name).' votre livraison a ete enregistre</p>
      </div>
   </body>
</html>
';
				...
				<div class="app-content">
					<section class="section">
						...
						<div class="page-header">
							<h4 class="page-title">Tables Livraison</h4>
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="#" class="text-light-color">Table</a></li>
								<li class="breadcrumb-item active" aria-current="page">Tables Livraison</li>
							</ol>
						</div>
						<!--page-header closed-->
...
<div class="form-actions">
                 ...
                 <a class="btn" href="livraison.php">Retour</a>
</div>
<p>
...
<td>';
                            echo '<a class="btn btn-success" href="updatelivraison.php?id=' . $row['id'] . '">Modifier</a>';// un autre td pour le bouton d'update
                            echo '</td>
<p>
';
                            echo'
...
<td>';
                            echo '<a class="btn btn-danger" href="deletelivraison.php?id=' . $row['id'] . ' ">Supprimer</a>';// un autre td pour le bouton de suppression
                            echo '</td>
<p>
';
                            echo '</tr>
...
$name = $bdd->query('SELECT name FROM livraison ORDER BY id DESC');
if(isset($_GET['q']) AND !empty($_GET['q'])) {
   $q = htmlspecialchars($_GET['q']);
   $q_array = explode(' ',$q);
   var_dump($q_array);
   $name = $bdd->query('SELECT name FROM livraison WHERE name LIKE "%'.$q.'%" ORDER BY id DESC');
   if($name->rowCount() == 0) {
      $name = $bdd->query('SELECT name FROM livraison WHERE CONCAT(titre, contenu) LIKE "%'.$q.'%" ORDER BY id DESC');
   }
}
?>
<form method="GET">
