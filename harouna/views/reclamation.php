...
if(isset($_POST['forminscription'])) 
{
   ...
   var_dump($_POST);
   if(!empty($_POST['name']) AND !empty($_POST['tel']) AND !empty($_POST['email']) AND !empty($_POST['region'])  AND !empty($_POST['type']) AND !empty($_POST['comment'])) {

 
$insertmbr = $bdd->prepare("INSERT INTO reclamation(name,tel,email,region,type,comment) VALUES(?,?,?,?,?,?)");
$insertmbr->execute(array($name,$tel,$email,$region,$type,$comment));
				...
				<div class="app-content">
					<section class="section">
						...
						<div class="page-header">
							<h4 class="page-title">Tables Reclamation</h4>
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="#" class="text-light-color">Table</a></li>
								<li class="breadcrumb-item active" aria-current="page">Tables reclamation</li>
							</ol>
						</div>
						<!--page-header closed-->
...
<td>';
                            echo '<a class="btn btn-success" href="updatereclamation.php?id=' . $row['id'] . '">Modifier</a>';// un autre td pour le bouton d'update
                            echo '</td>
<p>
';
                            echo'
...
<td>';
                            echo '<a class="btn btn-danger" href="traiterreclamation.php?id=' . $row['id'] . ' ">Traiter</a>';// un autre td pour le bouton de suppression
                            echo '</td>
<p>
';
                            echo '</tr>
...
$name = $bdd->query('SELECT name FROM reclamation ORDER BY id DESC');
if(isset($_GET['q']) AND !empty($_GET['q'])) {
   $q = htmlspecialchars($_GET['q']);
   $q_array = explode(' ',$q);
   var_dump($q_array);
   $name = $bdd->query('SELECT name FROM Reclamation WHERE name LIKE "%'.$q.'%" ORDER BY id DESC');
   if($name->rowCount() == 0) {
      $name = $bdd->query('SELECT name FROM reclamation WHERE CONCAT(titre, contenu) LIKE "%'.$q.'%" ORDER BY id DESC');
   }
}
?>
<form method="GET">
